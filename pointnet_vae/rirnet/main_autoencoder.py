...
class Model:
    def __init__(self, model_dir):
        ...
        self.autoencoder_optimizer = optim.Adam(self.autoencoder.parameters(), lr=self.autoencoder_args.lr, betas=(0.5, 0.99), eps=1e-5, weight_decay=0, amsgrad=False)
    ...
    def train(self):
        ...
        for batch_idx, (source, target) in enumerate(self.train_loader):
            ...
            autoencoder_loss_kld = self.kld(mu, logvar)/100
            ...
            autoencoder_loss_list.append(autoencoder_loss_chamfer.item())
            ...
            if batch_idx % self.autoencoder_args.log_interval == 0:
                print('Train Epoch: {:5d} [{:5d}/{:5d} ({:4.1f}%)]\tLoss: {:.4f}\t{:.4f}\t{:.4f}'.format(
                    ...
                    100. * batch_idx / len(self.train_loader), autoencoder_loss_chamfer.item(), autoencoder_loss_mse.item(), autoencoder_loss_kld.item()))
