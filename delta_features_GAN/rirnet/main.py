...
class Model:
    ...
    def train(self):
        ...
        for batch_idx, (source, target) in enumerate(self.train_loader):
            ...
            train_loss_D = getattr(F, self.G_args.loss_function)(decision.squeeze(), Variable(torch.ones(source.data.size()[0]).cuda()))*0.05
            ...
            if batch_idx % self.G_args.log_interval == 0:
                print('Train Epoch: {:5d} [{:5d}/{:5d} ({:4.1f}%)]\tLoss: {:.6f}, {:.6f}'.format(
                    ...
                    100. * batch_idx / len(self.train_loader), train_loss_G.item(), train_loss_D.item()))
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch_idx, (source, target) in enumerate(self.eval_loader):
                source, target = source.to(self.device), target.to(self.device)
                output = self.G(source)
                eval_loss = getattr(F, self.G_args.loss_function)(output, target).item()
