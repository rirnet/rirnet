...
class Model:
    def __init__(self, model_dir):
        ...
        self.autoenc.load_state_dict(torch.load(os.path.join(model_dir, '323_autoenc.pth')))
        self.extractor.load_state_dict(torch.load(os.path.join(model_dir, '73_extractor.pth')))
    ...
    def run(self):
        ...
        with torch.no_grad():
            for batch_idx, (source, target) in enumerate(self.eval_loader):
                ...
                filled_times_target, filled_alphas_target = self.fill_peaks(target[0,:], target[1,:])
                
                plt.subplot(211)
                plt.plot(filled_times_output, filled_alphas_output, 'x')

                plt.subplot(212)
                plt.plot(filled_times_target, filled_alphas_target, 'x')
                plt.show()


                output_rir = self.reconstruct_rir(filled_times_output, filled_alphas_output)
                target_rir = self.reconstruct_rir(filled_times_target, filled_alphas_target)
