...
data_folder = 'data'
header=['y_path', 'h_path', 'x_path', 'room_corners', 'room_absorption', 'room_mics', 'room_source']
...
def generate_waveforms(wav, h_list):
    ...
    for i_h, h in enumerate(h_list):
        ...
        wav_list.append(wav)
    return np.array(target_list), np.array(data_list), np.array(wav_list)
...
def build_db(root):
    ...
    wavs = load_wavs(audio_path, db_setup)
    ...
    while rir_generator.i_total < rir_generator.n_total:
        for h_list, info_list in rir_generator:
            ...
            target_list, data_list, wav_list = generate_waveforms(wav, h_list)
            ...
            with open(db_csv_path, 'a') as csvfile:
                ...
                for i, data in enumerate(data_list):
                    ...
                    data_filename = '{}_y.npy'.format(n_saved + i)
                    target_filename = '{}_h.npy'.format(n_saved + i)
                    wav_filename = '{}_x.npy'.format(n_saved + i)
                    data_path = os.path.join(data_folder_path, data_filename)
                    target_path = os.path.join(data_folder_path, target_filename)
                    wav_path = os.path.join(data_folder_path, wav_filename)
                    np.save(data_path, data)
                    np.save(target_path, target)
                    np.save(wav_path, wav)
                    writer.writerow([data_path, target_path, wav_path, corners, absorption, mics, sources])
