...
import sys
from compressor_database import RirnetDatabase
...
class Model:
    def __init__(self, model_dir):
        ...
        net_rirify = import_module('net_rirify')

        self.model = net.Net()
        self.model_rirify = net_rirify.Net()
        ...
        self.model_rirify.to(self.device)
        ...
        self.optimizer_rirify = optim.SGD(self.model_rirify.parameters(), lr=self.args.lr, momentum=self.args.momentum)

        if not list_epochs:
            epoch = 0
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> NOT ADDED, EPOCH MUST BE 0
        else:
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    def train(self):
        ...
        loss_list = []
        train_loss2 = 0
        for batch_idx, (y, h, x) in enumerate(self.train_loader):
            if batch_idx % 2 == 0:
                x = h.clone()
            x_prev = torch.tensor(np.zeros((len(x), 1024))).unsqueeze(1)
            x = x.to(self.device)
            train_loss = 0
            song = np.zeros([1])
            self.optimizer.zero_grad()

            n_parts = np.shape(x)[1]
            for i in range(n_parts):
                x_now = x[:,i,:].unsqueeze(1)
                x_prev = x_prev.type_as(x_now)
                x_now = torch.cat((x_prev, x_now), 1)
                output = self.model(x_now, compress=True, decompress=True)
                if self.epoch%10 == 0 and batch_idx == 10:
                    s = x_now.clone()
                    o = output.clone()
                    s = s[:,1,:].detach().unsqueeze(1).cpu().numpy()
                    o = output.detach().cpu().numpy()
                    print('plotting')
                    plt.subplot(2,3,1)
                    plt.plot(s[0,:,:].T)
                    plt.title('source, size')
                    plt.subplot(2,3,2)
                    plt.plot(s[1,:,:].T)
                    plt.title('source, size')
                    plt.subplot(2,3,3)
                ...
                #if train_loss2 == 0:
                facit = x_now[:,1,:].unsqueeze(1)
                train_loss = getattr(F, self.args.loss_function)(output, facit)
                #else:
                #    train_loss = getattr(F, self.args.loss_function)(output, x_now[:,1,:].unsqueeze(1)) + train_loss2.item()/100
                train_loss.backward()
                self.optimizer.step()

                prev = x_now[:,1,:].unsqueeze(1)
                song = np.concatenate([song,output[0,0,:].cpu().detach().numpy()])
                #song = np.concatenate([song,x_now[0,1,:].cpu().detach().numpy()])

            for ba in range(10):
                if True:
                    y_ = y[ba,:,:]
                    y_ = y_.to(self.device)
                    (n_pieces, _) = y_.data.size()
                    self.optimizer_rirify.zero_grad()

                    for m in range(n_pieces):
                        y_piece = y_[m,:]
                        if m == 0:
                            y_prev = y_piece.clone()*0
                        else:
                            y_prev = y_[m-1,:].clone()
                        y_piece = torch.cat((y_prev.unsqueeze(0), y_piece.unsqueeze(0)), 0).unsqueeze(0)
                        if m == 0:
                            y_ab = self.model(y_piece, compress=True, decompress=False).detach()
                        else:
                            y_ab = torch.cat((y_ab, self.model(y_piece, compress=True, decompress=False).detach()),1)

                    y_ab = Variable(y_ab)
                    rir_y = self.model_rirify(y_ab)
                    rir_y = rir_y.view(32,128)

                    for m2 in range(32):
                        if m2 == 0:
                            out = self.model(rir_y[m2,:], compress=False, decompress=True)
                        else:
                            out = torch.cat((out, self.model(rir_y[m2,:], compress=False, decompress=True)), 0)

                    h = h.to(self.device)
                    train_loss2 = getattr(F, self.args.loss_function)(out, h[ba,:,:].view(32768, 1))
                    train_loss2.backward(retain_graph=True)
                    self.optimizer_rirify.step()

                if (self.epoch%1 == 0 and batch_idx == 10):
                    s = out.view(32768).clone()
                    o = h[ba,:,:].view(32768).clone()
            ...
            #au.save_wav('t.wav', song, 44100)
            loss_list.append(train_loss.item())
            if batch_idx % self.args.log_interval == 0:
                print('Train Epoch: {:5d} [{:5d}/{:5d} ({:4.1f}%)]\tLoss: {:.6f}'.format(
                    self.epoch + 1, batch_idx * len(x), len(self.train_loader.dataset),
    ...
    def evaluate(self):
        ...
        eval_loss_list = []
        with torch.no_grad():
            for batch_idx, (sources, targets) in enumerate(self.eval_loader):
