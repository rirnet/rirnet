    ...
    def run(self):
        ...
        with torch.no_grad():
            for batch_idx, (source, target) in enumerate(self.eval_loader):
                ...
                plt.plot(output[0]*44100/1000, output[1], 'o', markersize = 0.5, label='Network output')
                plt.plot(filled_times_output*44100/1000, filled_alphas_output, 'o', markersize = 0.5, label='Extrapolated data')
                plt.plot(linex*44100/1000, liney, label='Approximated function')
                plt.grid()
                plt.xlabel('Time [ms]')
