...
class Model:
    def __init__(self, model_dir):
        ...
        self.discriminator = discriminator.Net()
        ...
        #self.extractor_optimizer = optim.SDG(self.extractor.parameters(), lr=self.extractor_args.lr, momentum=self.extractor_args.momentum, nesterov=True)
        ...
        self.discriminator_optimizer = optim.Adam(self.discriminator.parameters(), lr=self.discriminator_args.lr, betas=(0.9, 0.99), eps=1e-5, weight_decay=0, amsgrad=False)
        ...
        if list_epochs_extractor == []:
            ...
            self.discriminator_optimizer.load_state_dict(torch.load(os.path.join(model_dir, '{}_opt_disc.pth'.format(str(epoch)))))
            self.discriminator.load_state_dict(torch.load(os.path.join(model_dir, '{}_disc.pth'.format(str(epoch)))))
            for g in self.discriminator_optimizer.param_groups:
                g['lr'] = self.discriminator_args.lr
...
##############################################################################
    ...
    def generate_plot(self):
        ...
        plt.subplot(3,2,3)
        plt.plot(self.latent_target_im_train[:,0], self.latent_target_im_train[:,1], '--o', linewidth=0.5, markersize=2, label='target')
        plt.plot(self.latent_output_im_train[:,0], self.latent_output_im_train[:,1], '--x', linewidth=0.5, markersize=2, label='output')
        plt.grid(True)
        plt.legend()
        plt.title('Train latent')

        plt.subplot(3,2,4)
        plt.plot(self.latent_target_im_eval[:,0], self.latent_target_im_eval[:,1], '--o', linewidth=0.5, markersize=2, label='target')
        plt.plot(self.latent_output_im_eval[:,0], self.latent_output_im_eval[:,1], '--x', linewidth=0.5, markersize=2, label='output')
        plt.grid(True)
        plt.legend()
